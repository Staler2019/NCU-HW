...
#define MAX_MONEY 50001
    CASE
    {
        ...
        vector<int> kinds(n), money(s + 1, MAX_MONEY); // money.size():=(s+1)
        cin >> kinds;
        // cerr << kinds << endl;
        money[0] = 0;

        for (int i = 0; i < s + 1; i++) { // O(s)
            for (int &j : kinds) {        // O(n)
                if ((i + j) <= s) {
                    money[i + j] = min(money[i] + 1, money[i + j]);
                }
            }
        }

        // version2: wrong time complexity (n^2): should be (s*n)
        // for (int k = 0; k < s; k++) { // O(s)
        //     cerr << "money cage: " << money << endl;
        //     for (int &i : kinds) { // O(n)
        //         cerr << " i(in kinds)=" << i << endl;
        //         for (int j = 0; (k + j * i) <= s; j++) // O(n/s)  // dirty
        //             money[k + j * i] = min(money[k + j * i], j + money[k]); // dirty
        //     }
        // }
