...
int Solving()
{
    int n;
    while (cin >> n && n) {
        map<int, int> values; // value|index
        for (int k = 0; k < n; k++) {
            vector<int> length(2);
            cin >> length;
            sort(all(length));
            ...
            int value = 0;
            while (length[0] >= (length[1] / 2)) {
                length[1] *= 2;
                length[0] /= 2;
                cerr << "  length[]:" << length << endl;
            }
            if (length[0] >= (length[1] / 4))
                value = length[1] / 4;
            else
                value = length[0];
            // now length[0] >= length[1]/4
            // if(initial) length[0] < length[1]/4
            cerr << " k:" << k << " value:" << value << endl;
            if (values.find(value) == values.end())
                values[value] = k;
        }
        cout << (values.rbegin()->second) + 1 << endl;
    }
    Be AC; // End of code
}
int main()
